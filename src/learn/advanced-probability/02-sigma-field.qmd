...
以上两者被统称为**单调序列**，**单调序列总有极限**。我们还可以定义**上极限** ${\lim\inf}_{ n \to \infty }A_{n}:=\cup^{\infty}_{n=1}\cap^{\infty}_{k=n}A_{k}$ 和**下极限** $\lim\sup_{ n \to \infty }A_{n}:=\cap^{\infty}_{n=1}\cup^{\infty}_{k=n}A_{k}$，它们总是存在的。如果 ${\lim\inf}_{ n \to \infty }A_{n}=\lim\sup_{ n \to \infty }A_{n}$，则认为序列 $\{ A_{n} \}$ 的**极限**存在，其极限是 $\lim_{ n \to \infty }A_{n}:={\lim\inf}_{ n \to \infty }A_{n}=\lim\sup_{ n \to \infty }A_{n}$。
**Definition: 其他集合系的定义**
我们首先定义集合系是 sample space 子集的集合。
1. 如果某个非空集合系 $\mathcal{J}$ 满足 $A,B\in\mathcal{J}\implies A\cap B\in\mathcal{J}$，则称之为 **${\pi}$-class**。
2. 如果存在某个 $\pi$-class $\mathcal{I}$ 满足下面的条件，则称之为 **semi-ring**：
$\forall A,B\in\mathcal{I},A\subset B\implies \exists \{ C_{k}\in\mathcal{I},k=1,\dots,n \},\text{其中}\forall C_{i},C_{j},C_{i}\cap C_{j}=\emptyset,\text{使得}B\setminus A=\cup^{n}_{k=1}C_{k}$
3. 如果某个非空集合系 $\mathcal{R}$ 对并和差运算是封闭的，则称之为 **ring**:
$A,B\in\mathcal{R}\implies A\cup B,A\setminus B\in\mathcal{R}$
4. 若某个非空集合系 $\mathcal{R}$ 对可列并和差运算封闭，则称之为 **$\sigma$ -ring**:
...
2. 如果集合系$\mathcal{M}$中的任意单调序列$\{ A_{n},n=1,2,\dots \}$，均有$\lim_{ n \to \infty }A_{n}\in\mathcal{M}$，则称之为**monotone-class**。
