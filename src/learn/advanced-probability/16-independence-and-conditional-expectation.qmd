...
[随机向量的分量可测性判定](15-random-vectors.qmd#thm-vector-measurable)的证明中已经得到该式。而 $\sigma(g(f_{1},\dots))$ 是 $\sigma((f_{1},\dots))$ 的子集，因此自然也是独立的。
:::
以上推论告诉我们，**对于相互独立的随机变量，将其任意分割成多个组，然后对每个组进行任意的变换得到的新的随机变量，依然是相互独立的。**

@thm-independent-pi-class 提示我们，对于随机变量的独立性判定，可以只通过分布函数 $F$ 进行；这里使用了[联合分布函数](15-random-vectors.qmd#def-n-dist-rv)与[边缘分布函数](15-random-vectors.qmd#def-margin-dist)之间的关系。也就是下面的一系列等价条件：
...
若是 $Pf^{-1}=Pg^{-1}$，则独立是有可能成立的。比如，在 Borel 乘积测度空间中 $(\mathbb{R}^{n},\mathcal{B}^{n},P^{n})$，对于其上所有的 [投影](13-finite-product-spaces.qmd#def-project) $\pi_{i}$ ，此时，容易证明其满足 @thm-indepent-distribution (2) ，因此满足独立的定义。而其分布函数是 $P^{n}\pi_{k}^{-1}=P$ 是一样的。
...
其中，第一行来自给定值条件期望的定义，第三行使用 Fubini 定理，最后一行使用[联合密度与边缘密度的关系](15-random-vectors.qmd#prp-comb-margin-density)。
:::
